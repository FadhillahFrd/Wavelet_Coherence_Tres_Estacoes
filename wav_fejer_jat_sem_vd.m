...
% Wavelet coherence: parâmetros ionosféricos × (PPEF, DDEF)
clc; clear; close all;
...
Wplot = Wplot / max(Wplot(:));   % agora vai de 0 a 1
        % --- Plot ---
        wcoh_central_norm = wcoh_central / max(wcoh_central(:));  % escala de 0 a 1
...
h = pcolor(datenum(iono_time), period_lin, Wplot);
        ...
        xticks_custom = datenum(datetime(2017,8,1):days(2):datetime(2017,9,1));
...
ax.YMinorTick = 'off';            % desliga os minor ticks
        ...
        title([nome_iono ' × ' nome_vd ' - Jataí (GO)'], ...
...
outputFolder = 'figuras_wcoh_jat';
if ~exist(outputFolder, 'dir')
    mkdir(outputFolder);
end

% --- Criar nome do arquivo (sem espaços nem caracteres especiais)
filename = sprintf('WCOH_JAT_%s_%s', nome_iono, nome_vd);
